...
export default function HistoryPage() {
  ...
  const onDelete = async (id: string) => {
    if (!window.confirm(translate('history.confirmDelete'))) return;
    try {
  ...
  return (
    <section className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-slate-900">{translate('history.title')}</h1>
        <p className="text-sm text-slate-500 mt-1">{translate('history.subtitle')}</p>
      </div>
