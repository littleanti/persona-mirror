...
async def get_session(
    response: Response,
    session_id: Optional[str] = Cookie(default=None, alias="persona_session"),
) -> Tuple[Path, Path]:
    """쿠키에서 세션 ID를 읽거나 새로 발급한다. (personas_dir, analyses_dir) 반환."""
    if not session_id or not _UUID_RE.match(session_id):
        session_id = str(uuid.uuid4())

    # 매 요청마다 쿠키 만료일을 갱신 (1년)
    response.set_cookie(
        key="persona_session",
        value=session_id,
        max_age=365 * 24 * 3600,
    ...
    personas_dir = SESSIONS_DIR / session_id / "personas"
    analyses_dir = SESSIONS_DIR / session_id / "analyses"
    personas_dir.mkdir(parents=True, exist_ok=True)
    analyses_dir.mkdir(parents=True, exist_ok=True)
    return personas_dir, analyses_dir
...
async def list_personas(
    dirs: Tuple[Path, Path] = Depends(get_session),
):
    personas_dir, _ = dirs
    personas = []
...
async def analyze_message(
    ...
    personas_dir, analyses_dir = dirs
...
{{
  ...
  "candidates": [
    ...
    (analyses_dir / f"{analysis_id}.json").write_text(
