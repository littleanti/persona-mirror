...
M1(P4 완료) 시점에 실재하는 파일에 **P5에서 만들 `lib/image.ts` 한 개를 더한 것**이다. 1.0에서 `src/` 실제 트리와 대조했고, `image.ts`를 뺀 나머지는 저장소 내용과 일치한다. 괄호는 생성 단계이며 `(P5)`는 아직 없는 파일이라는 뜻이다. P6 이후 추가분은 §3의 해당 단계 산출물에만 적고 이 트리에는 넣지 않는다(확정 시 갱신).
...
└── src/
    ...
    └── lib/
        ...
        ├── types.ts              # PersonaFields · PersonaRecord · PersonaSummary · CreatePersonaInput · CandidateReply · AnalysisRecord   (P3, Analysis 타입은 P4, InlineImage·images?는 P5)
...
| 캡처 페르소나의 정확도(텍스트 대비)·장수 상한·이미지 압축 | **미확정** — 캡처 한 장이 담는 발화가 적을 수 있다는 지적을 반증하지 못했다(TRD ADR-6). 상한과 리사이즈는 두지 않고 시작하며 지연 실측 뒤 판단 | P5 관찰 → P6 실사용 |
