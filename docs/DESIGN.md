...
> 문서 버전: 1.3 · 갱신일: 2026-09-05 · 상태: P7 안정화 착수 — 오버레이 닫기 판정·포털 규칙 확정, U17 원인 확정
...
| ~~U17~~ | 상세 모달 백드롭이 최상단 약 20px를 덮지 않음 | **원인 확정(P7-3)**: `space-y-5` 부모의 margin-top 주입. 페이지 오버레이는 `createPortal(document.body)`로 렌더(§2.6 규칙) | P7-3 진행중 |
