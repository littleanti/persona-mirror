...
// 휴대폰 등에서 LAN IP(http://192.168.x.x)로 접속하면 비보안 컨텍스트가 되어
// crypto.randomUUID가 undefined라 "is not a function"으로 실패한다.
// 따라서 randomUUID → getRandomValues → Math.random 순으로 단계적으로 폴백한다.

function fromRandomBytes(bytes: Uint8Array): string {
  // RFC 4122 v4: version(4)·variant(10) 비트를 강제로 설정
...
export function uuid(): string {
  ...
  // 3) 최후 폴백: Math.random(암호학적으로 안전하지 않지만 id 충돌 방지 용도로는 충분)
