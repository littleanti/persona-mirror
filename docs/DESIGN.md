...
> 문서 버전: 1.8 · 갱신일: 2026-09-05 · 상태: P11 최종 — 독립 리뷰 반영(§1.1(A)·§5.2·§5.3·§6.1·§7b·§9 코드 기준 정정, U14 종결, U36·U37)
...
| 1.8 | 2026-09-05 | P11 리뷰 반영: 생성 시트 닫기 시 입력 유지·저장 중 가드 없음(§1.1(A)·§5.2·§9·U14), §5.3 필드 순서, §6.1 피커 라벨 폴백 단서, §7b 고지 문구, 문서 이력 순서 정렬 |
...
- 비판적 재사고: 카카오톡 대화는 수백 줄을 붙여넣는다. 시트 내부 스크롤 + 소프트 키보드가 겹치면 textarea가 가려질 수 있다. 또 실수로 백드롭을 누르면 시트가 닫히는데, 그때 긴 붙여넣기까지 잃으면 비용이 크다.
- 종합: 시트를 채택하되 ① 높이를 `h-[92dvh]`로 고정하고 본문만 `overflow-y-auto`, textarea는 `flex-1`로 남은 높이를 채운다 ② 백드롭·X로 닫아도 **입력을 버리지 않는다** — 시트 컴포넌트(`CreatePersonaDialog`)는 `open`이 거짓이면 `null`을 렌더할 뿐 마운트된 채 남아 이름·나의 이름·대화·첨부 안내가 유지되고, 다시 열면 그대로 복원된다. 폐기되는 시점은 생성 성공 직후(`reset`)와 탭 전환(페이지 언마운트) 둘뿐이다. 그래서 2차가 지적한 오클릭 유실은 같은 탭 안에서는 일어나지 않으며, 확인 대화상자는 두지 않는다(§12 U14 종결). 생성 중(`saving`)에도 닫기를 막지 않는다 — 닫혀도 요청은 이어지고, 성공하면 토스트·초기화·목록 재조회가 그대로 일어난다(P11 코드 대조로 확인한 동작). 키보드 겹침의 실기기 동작은 **미확정**(P3 모바일 스모크에서 확인, 문제 시 시트 높이를 키보드 높이에 맞춰 조정).
...
│░░░  │        환영합니다            │  ░░░░░│
│░░░  │  상대방의 페르소나에 맞는    │  ░░░░░│  text-sm slate-500 (onboarding.welcomeDesc)
│░░░  │  답장을 찾아드려요.          │  ░░░░░│
│░░░  │                              │  ░░░░░│
...
│░░░  │                              │  ░░░░░│
│░░░  │  □ 대화 내용이 Gemini API로  │  ░░░░░│  checkbox accent-indigo (onboarding.consent)
│░░░  │    전송되고, 로컬 데이터는   │  ░░░░░│
│░░░  │    … 복구할 수 없음을 이해…  │  ░░░░░│
│░░░  │                              │  ░░░░░│
...
| 로딩 | 버튼 `disabled` + 라벨 `페르소나 생성 중...`(`persona.create.loading`). 백드롭·X 닫기는 **막지 않는다**(§1.1 A) — 닫아도 컴포넌트가 마운트된 채 요청이 이어지고, 성공하면 토스트·폼 초기화·목록 재조회가 그대로 일어난다. 요청은 항상 텍스트 경로(60초 타임아웃)다 |
...
| 취소 | 닫기(X)·백드롭 → 시트만 닫힘. **입력은 폐기하지 않는다** — 컴포넌트가 마운트된 채 남아 같은 탭 안에서 다시 열면 이름·나의 이름·대화·첨부 안내가 복원된다(§1.1 A, §9). 폐기는 성공 행의 초기화와 탭 전환(언마운트) 때만. ESC는 없다(§12 U6) |
...
| 항목 | 규칙 |
|---|---|
| 필드 표시 순서 | summary는 상단 블록으로 따로 빼고, 나머지 카드는 **LLM 응답 JSON의 키 순서**대로 렌더한다(`Object.entries`). 프롬프트 스키마가 `PERSONA_FIELDS` 순(communication_style → speech_level → vocabulary_examples → sentence_style → emoji_symbol_usage → texting_habits → emotional_tendencies → what_they_value → how_they_seek_response → relationship_dynamics)이라 보통 그 순서와 일치하지만 화면이 강제하지는 않으며, 알 수 없는 키도 응답에 나온 자리에 그대로 끼어든다(P11 코드 대조). 라벨은 i18n `persona.field.*` |
...
| 타겟 피커 | **텍스트 모드 전용.** 파싱된 라인이 하나라도 있을 때만 타겟 칩 아래에 접힘 토글 `analyze.pickTarget`("다른 메시지에 답장하기") + `▾`/`▲`. 펼치면 `max-h-40 overflow-y-auto space-y-1` 목록에 라인마다 버튼 하나: 화자 라벨(`slate-400 font-semibold`, 나=`my_name` 또는 "나" — 이 폴백은 ko 고정 문자열로 i18n 키가 없어 en 모드에서도 "나"로 보인다(§12 U36) / 상대=페르소나 이름 / 미상=원본 라벨 또는 `?`) + 본문 50자 컷. 현재 타겟인 항목은 `bg-indigo-50 text-indigo-700`. 고르면 즉시 반영되고 피커는 닫힌다 |
...
│ │ ● 생성 시 대화 텍스트나 캡처 이미지가, │ │  settings.privacyGemini — P5~P9 문구 잔존(U37)
│ │   분석 시 페르소나와 대화 내용이       │ │
│ │   Google Gemini API로 직접 전송됩니다  │ │
│ │ ● 키는 localStorage에 저장됩니다.      │ │  settings.privacyKey — referrer 절 잔존(U37)
│ │   Gemini API만 허용하고 가능하면 HTTP  │ │
│ │   referrer를 …github.io로 제한하세요   │ │
│ │ ● 브라우저 데이터 삭제·기기 변경 시    │ │
│ │   복구할 수 없습니다. 백업하세요       │ │
│ │ ● 타인의 대화·민감정보 입력에 주의     │ │
│ └────────────────────────────────────────┘ │
...
| 개인정보 카드 | 항목마다 `flex gap-3` + 좌측 emerald 점(`mt-2 h-1.5 w-1.5 rounded-full bg-emerald-500`). 다섯 항목은 PRD FR-38의 ①~⑤이고, ⑥(AI 결과는 참고용)은 톤이 달라 카드 3으로 분리했다. **문구 2건이 계약과 어긋난다(P11 코드 대조, §12 U37)** — ②(`settings.privacyGemini`)는 캡처를 페르소나 생성 쪽에 붙이고 분석 쪽에서 빼는 P5~P9 문구가 그대로 남아 있다(PRD DR-4: P10 이후 캡처를 보내는 화면은 분석 하나). ③(`settings.privacyKey`)은 "HTTP referrer를 littleanti.github.io로 제한" 권고를 담고 있는데, `no-referrer` meta([TRD §8.1](./TRD.md)) 때문에 이 앱은 Referer를 보내지 않으므로 그 제한을 걸면 Gemini 호출이 거부된다 — FR-38 ③은 사용 API 제한만 권한다. 두 키의 ko/en 문구 정정은 후속([PLAN §8.3](./PLAN.md) #4) |
...
| 입력 유지 | 탭 전환 시 각 탭은 언마운트되어 입력값·결과를 보존하지 않는다. 생성 시트는 닫아도 입력을 유지한다(같은 탭 안에서 다시 열면 복원 — §5.2 취소 행); 탭을 떠나면 페이지와 함께 사라진다. **예외 하나** — 분석 탭의 최근 대화 스레드는 페르소나별 드래프트로 저장돼 다시 들어오면 복원된다(§6.3). 의도 선택·수동 타겟·결과 카드는 저장하지 않는다: 의도와 타겟은 그때그때 달라지는 값이고, 결과는 기록 탭에 이미 남는다 |
...
**취소선이 그어진 항목은 종결**(결과만 남긴다), 나머지는 **남은 미확정**이며 확정 방법·시점을 함께 적는다. P11 기준으로 종결된 것은 U1·U2·U3·U14·U17·U18·U19·U20 여덟이고, 나머지는 실사용 관찰이나 실기기 확인을 기다린다. 화면 밖 항목까지 포함한 단일 목록은 [`./PLAN.md`](./PLAN.md) §8에 있다.
...
| ~~U14~~ | 생성 시트 백드롭 오클릭으로 입력 유실(§1.1 A) | **종결(P11 코드 대조)** — 구현은 시트를 닫아도 입력을 버리지 않는다(컴포넌트가 마운트된 채 `null`을 렌더). 같은 탭 안에서 다시 열면 복원되므로 오클릭 유실은 일어나지 않는다. 확인 대화상자는 두지 않는다. 탭을 떠나면 사라지는 것은 §9 "입력 유지"의 일반 규칙이다 | 완료 |
...
| U36 | 타겟 피커 내 발화 라벨 폴백 "나"(§6.1) | **미해결(P11 코드 대조).** `my_name`이 비면 라벨이 ko 고정 문자열 "나"로 떨어져 en 모드에서도 한글이 보인다 — §10.1에 키가 없다(P6-2 LOG가 미확정으로 남겼으나 이 표에서 빠져 있었다). 같은 성격의 ko 고정 `Error` 메시지 2곳(TRD §10 #33)과 함께 `analyze.me`(ko "나"/en "Me") 키로 뺀다 | U34와 묶어 후속 |
| ~~U37~~ | 설정 탭 개인정보 카드 문구 2건(§7b) | **미해결(P11 코드 대조).** `settings.privacyGemini`는 캡처를 페르소나 생성 쪽에 붙인 P5~P9 문구가 남아 있고(PRD DR-4), `settings.privacyKey`는 FR-38 ③에 없는 "HTTP referrer 제한" 권고를 담는다 — `no-referrer` meta(TRD §8.1) 때문에 걸면 Gemini 호출이 거부된다. 두 키 ko/en을 FR-38 ②③대로 고친다(문서 먼저 → `i18n.ts`) | 후속 후보([PLAN §8.3](./PLAN.md) #4) → **종결(P11 ④)**: `settings.privacyGemini`·`settings.privacyKey` 문구를 코드에서 정정 |
