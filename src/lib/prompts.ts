...
/**
 * 답장 의도 키 → 프롬프트 디렉티브 문장(TRD §3.5).
 * 빈 문자열이면 null(의도 미지정 = 공감 3축 기본). 프리셋 키가 아니면 자유 입력으로 보고 그대로 쓴다.
 */
...
/**
 * 메시지 분석 프롬프트 빌더(v2: 최근 대화 스레드 + 답장 대상 + 답장 의도, TRD §3.5).
 * 정적 페르소나(장기) + 최근 대화 스레드(단기 맥락) + 답장 대상(타겟) + 답장 의도를 받아
 * 심리 분석 + 답장 후보 3개를 요청한다. 의도가 비면 v1과 완전히 같은 공감 3축을 유지한다(무회귀).
 */
