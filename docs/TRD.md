...
> 문서 버전: 2.4 · 갱신일: 2026-09-05 · 상태: P11 최종 — 독립 리뷰 반영(§3.1/§3.5/§3.7/§3.8/§3.9/§3.10 코드 기준 정정, §10 #33·#34). 기준: [PRD 1.6](./PRD.md) / [PLAN 2.5](./PLAN.md) / [DESIGN 1.7](./DESIGN.md)
...
| 2.3 | 2026-09-05 | P11 최종 동기화: 머리말 주석의 "아직 코드에 없다" 서술 종결, §3.0 트리에 `vite-env.d.ts`, §3.8 코드 블록의 래퍼 주석 잔여 제거, §4 요청 경로 표에 P6~P10 지연 실측 반영, §5 ADR-7 종합의 래퍼 존치 서술 정정, §6.2 로컬 서버 코드 블록을 base 마운트 실제 구현으로 정정(§6.3·§7·§9.4 접속 주소 포함), §8 npm audit·§8.1 dev 충돌 서술을 P8 결과로 정정, §9.7~§9.9 제목을 실행 완료로, **신규 §9.10 최종 검증 요약**, §10 #12·#23·#29·#30 정정 및 최종 정리 |
| 2.4 | 2026-09-05 | P11 리뷰 반영: §3.1 AnalyzeReplyInput 문서용 계약 명시, §3.5 프롬프트 블록 순서, §3.7 splitPersonaRaw 사용처, §3.8 slice(0,3), §3.9 initI18n, §3.10 시트 입력 유지·not-found 문구, §10 #33·#34 |

> **P11 기준으로 §3의 모든 식별자는 `src/` 아래에 실재한다.** 이전 판에서 "아직 코드에 없다"고 밝혀 두었던 계약 — 1.3의 분석 재설계(`thread.ts`, `drafts.ts`, `analyzeReply`, `updatePersona`, `buildAnalyzePrompt` v2, `AnalysisRecord`의 선택 필드 3개, `ReplyIntentKey`·`REPLY_INTENTS`·`AnalyzeReplyInput`, `PersonaRecord.updated_at`), 1.6의 P8(`dataManagement.ts`, `assets.ts`, `SettingsPage`, `drafts.ts`의 백업 헬퍼 3종, `config.ts`의 저장소 키 상수 교체, localStorage 기반 `settingsRepo`), 1.8의 P9(`AnalyzeReplyInput.images?`, `buildAnalyzePrompt`의 `useImages` 분기, `analyzeReply`의 이미지 경로와 플레이스홀더 저장, `AnalyzePage`의 입력 모드 토글), 2.0의 P10(`chatFile.ts`의 `parseKakaoChatTail`, `PERSONA_CHAT_TAIL_CHARS`, 텍스트 전용 `CreatePersonaInput`, 이미지 분기가 사라진 `buildPersonaPrompt`, 단일 흐름 `PersonaPage`) — 은 **모두 구현됐다.** 2.0이 예고한 삭감(`CreatePersonaInput.images`, 생성 시트의 입력 모드 세그먼트·드롭존·썸네일, `persona.create.tab*`·`image*` 키)도 코드에서 사라졌고, P11에서 `analyzeMessage` 래퍼까지 제거했다.
>
> 이 문서는 **현재 확정된 설계**를 서술한다. 변경 이력은 [`LOG.md`](./LOG.md)에만 적는다. §3의 시그니처는 모든 구현 작업이 따라야 하는 **계약**이며, 계약을 바꿀 때는 코드보다 이 문서를 먼저 갱신한다(CLAUDE.md 그라운드 룰 2). 1.0(M1)과 2.3(P11) 두 시점에 코드와 한 줄씩 대조해 어긋난 서술을 코드 기준으로 정정했다.
...
├── vite-env.d.ts       # Vite 클라이언트 타입 참조(import.meta.env)
...
└── lib/
    ...
    ├── image.ts        # fileToInlineImage — File → InlineImage (P5에서 생성, P10 이후 분석 탭 전용)
    ├── chatFile.ts     # parseKakaoChatTail — 카카오톡 .txt 머리말 제거 + 말미 컷 (P10에서 생성)
    ...
    ├── *.test.ts       # vitest 단위 테스트 — thread · gemini(extractJson) · drafts · id (P6~P7) · chatFile (P10)
    └── repos/          # settingsRepo(localStorage) · personaRepo · analysisRepo (IndexedDB)
```

의존 방향은 한 방향이다: `routes/components → lib/persona·analysis → lib/repos·gemini·prompts·thread → lib/db·config·types`. 화면 코드는 `repos`·`gemini`를 직접 호출하지 않는다(유스케이스를 경유). 단, `ApiKeyStatus`/`OnboardingModal`은 스토어를 통해 `settingsRepo`에 닿는다. `image.ts`는 `dom.ts`와 같은 층의 순수 헬퍼라 화면이 직접 import한다 — 파일 선택은 브라우저 이벤트라 화면에서만 일어나고, 유스케이스는 이미 변환된 `InlineImage[]`만 받는다(§3.4.1). **P10 이후 이 모듈의 호출부는 `AnalyzePage` 하나다.** `chatFile.ts`(§3.15)도 같은 자리에 놓인다 — `PersonaPage`가 읽은 파일 텍스트를 넘겨 잘린 문자열을 돌려받고, 유스케이스(`createPersona`)는 그 문자열이 어디서 왔는지 알지 못한다. 두 모듈의 대칭은 의도한 것이다: **파일을 만지는 일은 화면에서 끝내고, 도메인 계층은 이미 정제된 값만 받는다.**
 ...
 * P10 이후 이 타입을 쓰는 곳은 메시지 분석(AnalyzeReplyInput.images) 하나다.
 */
...
/**
 * 페르소나 생성 입력 — 텍스트 전용(PRD FR-7).
 * conversation에는 붙여넣은 대화 또는 .txt 첨부로 채워진 텍스트가 들어간다.
 * 화면이 파일을 읽어 parseKakaoChatTail(§3.15)로 잘라 넣으므로, 이 계약에서 첨부는 보이지 않는다.
 */
...
`AnalyzeReplyInput`은 **문서용 계약**이다 — `analyzeReply(personaId, input)`의 `input`은 이 인터페이스에서 `personaId`를 뺀 형태와 필드가 같지만, 코드는 §3.8의 시그니처처럼 **인라인 타입으로 선언하고 이 인터페이스를 import하지 않는다**(`AnalyzePage`도 객체 리터럴을 넘긴다). `types.ts`의 선언은 "분석 입력에 무엇이 들어가는가"를 한 곳에 적어 두는 용도이며, 두 선언이 어긋나지 않도록 필드를 바꿀 때는 둘을 함께 고친다(P11 코드 대조에서 확인한 상태).
...
**P10의 타입 변경은 이 문서에서 처음 나오는 "삭감"이다** — `CreatePersonaInput.images`를 없앤다. 지금까지의 변경은 모두 선택 필드 가산이었고 그 원칙은 저장 스키마(`PersonaRecord`·`AnalysisRecord`)에 대해서는 그대로 유지된다. 여기서 지우는 것은 **저장되지 않는 입력 DTO의 선택 필드**라 IndexedDB에 닿지 않는다 — `DB_VERSION`은 1 그대로이고, P5~P9 사이에 캡처로 만든 레코드도 그대로 읽힌다(그 레코드의 `conversation`에는 캡처 장수 플레이스홀더 문자열이 남아 있고, 상세 화면은 그것을 평범한 문자열로 표시한다). `InlineImage`는 **지우지 않는다** — 분석 탭이 계속 쓴다.
...
export const PERSONA_CHAT_TAIL_CHARS = 16_000;       // 첨부 대화 파일(.txt)에서 쓸 말미 글자 수 상한(P10에서 추가)
...
- 모델은 하나뿐이므로 `IMAGE_MODEL` 같은 상수는 두지 않는다. 이미지 입력이 바꾸는 것은 **타임아웃 하나**이며, 그래서 상수도 타임아웃만 늘렸다. 180초는 여유값이고, 실측은 각 표본 1건뿐이다 — 캡처 1장 페르소나 생성 4.95s(P5), 캡처 1장 분석 2.75s(P9)(§10 #15·#28). **P10 이후 이 타임아웃이 걸리는 경로는 분석 탭 하나다.**
- **`PERSONA_CHAT_TAIL_CHARS`는 UI 상수가 아니라 입력 상한이다**(P10). 값은 16,000이며 근거는 계산 하나다 — 샘플 대화의 평균 32.6자/메시지 기준 약 490개 메시지, UTF-8 약 46 KB([PRD §8 부속 결정 3](./PRD.md)의 재검토 표). **이 분량에서 토큰·지연·품질을 재 본 적은 없다**(§10 #29). 첨부 파일이 이 값보다 짧으면 컷은 일어나지 않는다. 상수를 `chatFile.ts` 안이 아니라 여기 두는 이유는 §3.15에 적는다.
...
#### 3.4.1 `src/lib/image.ts` — File → InlineImage (P5에서 생성, P10 이후 분석 탭 전용)
...
- **호출부는 P9에서 둘로 늘었다가 P10에서 다시 하나가 된다**(`AnalyzePage`만). 함수 자체는 P5 이후 한 글자도 바뀌지 않았다. `PersonaPage`가 이 모듈을 더 쓰지 않게 되어도 **모듈은 남는다** — 분석 탭의 캡처 모드가 계속 쓰기 때문이며, 같은 이유로 `InlineImage`·`generate(prompt, images?)`·`IMAGE_REQUEST_TIMEOUT_MS`도 그대로다(ADR-10).
...
- **입력 소스 블록은 하나다**(P10에서 분기 제거). `대화 기록:` 뒤에 `conversation`을 그대로 붙인다. 그 텍스트가 붙여넣은 것인지 .txt 첨부로 채워진 것인지는 **프롬프트가 알 필요가 없다** — 화면이 이미 정제해 넘긴 문자열 하나이기 때문이다(§3.10·§3.15).
  - P5~P9의 `input.images` 분기(캡처 이미지 지시)는 **삭제한다**. 페르소나 생성에 이미지 경로가 없어졌으므로 분기를 남기면 도달할 수 없는 코드가 된다(ADR-10).
  - `buildAnalyzePrompt`의 `useImages` 분기는 **그대로 둔다.** 두 빌더가 같은 파일에 있지만 서로 독립이며, 분석 탭의 캡처 모드는 유지되기 때문이다.
...
- 입력 블록(순서 고정): 상대 페르소나 JSON → 상대 말투 요약(`speech_level`, `vocabulary_examples` 앞 8개, `sentence_style`, `emoji_symbol_usage`, `texting_habits`가 있을 때만) → 나의 페르소나 JSON(있을 때만) → **최근 대화 흐름 블록** → **답장 대상 지시** → 분석 질문(말투·나의 페르소나 질문 포함) → 공감 가이드라인 → 후보 지침(의도 디렉티브가 있으면 여기서 분기) → 나의 말투 지시(있을 때만 — 나의 페르소나 JSON과 떨어져 후보 지침 **뒤**에 놓인다) → JSON 형식 → 언어 지시.
...
- **`useImages`가 참이면 위 두 블록만 갈아 끼운다**(P9에서 추가 — PRD FR-39 / ADR-9). 나머지 블록(페르소나 JSON·말투 요약·말투 지시·분석 질문·공감 가이드라인·의도 디렉티브·후보 3축·JSON 형식·언어 지시)은 두 모드가 **완전히 같다**. 출력 계약을 하나로 유지해 `analysis.ts`의 정규화·저장 코드가 분기하지 않게 하기 위함이다(P5~P9 동안 `buildPersonaPrompt`의 이미지 분기도 같은 원칙을 따랐다 — 그 분기는 P10에서 사라졌지만 원칙은 여기 남는다).
...
`createPersona` 흐름(P10 이후 **텍스트 전용**):
1. `buildPersonaPrompt(input, getLang())` — 분기 없이 `conversation` 하나를 입력 소스로 쓴다(§3.5)
2. `generate(prompt)` — 두 번째 인자를 넘기지 않으므로 항상 텍스트 요청(문자열 `contents` + 60초). 실패는 §4의 사용자 친화 Error로 그대로 전파(화면이 토스트)
3. `extractJson(text)` → `my_name`이 있으면 `raw.other_persona`/`raw.my_persona`로 분리(없으면 `raw` 전체를 상대 페르소나, `my_persona = {}`). 분리 헬퍼는 `splitPersonaRaw(raw, myName): { personaData, myPersonaData }`이며, 사용처가 같은 모듈의 `createPersona`·`updatePersona` 둘이라 **모듈 내부 함수로 두고 export하지 않는다**. 다른 모듈이 쓰게 되면 그때 export한다. **JSON 파싱 실패(`'raw' in result`)는 거부하지 않고 원문을 보존해 저장한다** — 상대 페르소나가 `{ raw: text }`가 되고, `PersonaFields`의 인덱스 시그니처 덕에 상세 화면이 알 수 없는 키를 관대하게 표시하므로 사용자는 원문을 보고 삭제 후 재시도할 수 있다. (3단 사고: 1차 — 거부가 깔끔하다 / 2차 — 거부하면 사용자가 얻는 것이 없고 실패 원인을 볼 수도 없다, 반대로 저장하면 쓰레기 레코드가 남지만 삭제 한 번으로 정리된다 / 종합 — 원문 보존 저장. 실패율은 실호출에서 관찰해 `responseMimeType` 도입 여부(§10 #2)의 근거로 쓴다.)
4. `{ id: uuid(), name, my_name(trim), created_at: now ISO, conversation, persona, my_persona }` 구성
5. `personaRepo.put(record)` → 반환

입력 검증(이름 필수, 키 있음, 대화 trim 20자 이상)은 화면(`PersonaPage`)이 호출 전에 수행한다. **모드별 분기가 없어져 검증도 한 줄기다**(§3.10).

**`conversation`에 무엇이 저장되나**(P10). `PersonaRecord.conversation`은 상세 모달의 "원본 대화 기록 보기"가 읽는 필드다(DESIGN §5.3). 저장되는 것은 **모델에 보낸 그 텍스트 그대로**다 — 붙여넣은 것이든 .txt 첨부로 채워진 것이든 화면 입력란의 값이 그대로 프롬프트로도 가고 레코드로도 간다. 따라서 **첨부로 만든 페르소나도 근거 대화를 전부 되짚어 볼 수 있다.** P5~P9의 캡처 모드는 이것이 불가능해 플레이스홀더 한 줄만 남겼는데, 그 규정은 P10에서 함께 사라졌다.

- **첨부한 파일은 저장하지도, 업로드하지도 않는다.** 화면이 `FileReader`로 읽어 `parseKakaoChatTail`(§3.15)로 자른 결과만 입력란에 들어가고, `File` 객체는 그대로 버려진다.
- **P5~P9 사이에 캡처로 만든 기존 레코드**의 `conversation`에는 플레이스홀더 문자열("[채팅 캡처 이미지 3장으로 생성된 페르소나]")이 남아 있다. 마이그레이션하지 않는다 — 그것도 그냥 문자열이고 상세 화면은 문자열을 그대로 보여줄 뿐이다. `DB_VERSION`은 1 그대로다.
...
이 경로에는 이미지 인자가 없었고 P10 이후에도 없다 — 이제 생성 경로와 **같은 모양**이 됐다(둘 다 텍스트 하나를 받아 `generate(prompt)`를 부른다). 다만 P5~P9 사이에 캡처로 만든 레코드를 업데이트하면 `conversation`이 "플레이스홀더 한 줄 + 새 대화"가 되어 그 상태 그대로 재분석된다 — 앞줄이 근거 없는 한 줄로 남는 상태이며 품질은 미확정이다(§10 #17). 상세 화면의 업데이트 입력란에는 **.txt 첨부를 두지 않는다**(§3.10).
...
6. 정상이면 `analysis`(string 아니면 ''), `candidates`(배열 아니면 [], 3개를 넘으면 앞 3개만 — `.slice(0, 3)`)로 정규화
...
**이미지 모드의 플레이스홀더**(P9 — PRD FR-40): 이 모드에는 앱이 아는 타겟 문장이 없어 `message`·`target_message`가 둘 다 빈 문자열이 되고, 그러면 기록 목록의 미리보기가 통째로 빈다. 그래서 캡처 장수를 담은 문자열을 같은 두 자리에 넣는다. `t()`를 **저장 시점에** 부르므로 그 언어로 굳고 나중에 UI 언어를 바꿔도 번역되지 않는다 — 저장 데이터는 생성 당시 언어를 유지한다는 PRD FR-27의 규칙이며, P5~P9 동안 페르소나 생성이 `persona.create.imagePlaceholder`를 같은 방식으로 다뤘다(그 키는 P10에서 사라졌지만, **이미 저장된 문자열은 값이라 그대로 남는다**). `thread`에는 `''`을 넣는다(붙여넣은 원문이 없으므로). **레코드 스키마는 바뀌지 않는다** — 기존 필드에 다른 문자열이 들어갈 뿐이라 기록 탭 코드도 `DB_VERSION`도 손대지 않는다.
...
export function initI18n(): void;                       // <html lang>·document.title 동기화. 초기 언어는 모듈 로드 시 detectInitialLang()(localStorage 'pm_lang' → navigator.language가 ko*면 ko, 아니면 en; navigator가 없으면 ko)이 결정
...
- **P9은 새 영역을 만들지 않는다.** 신규 키(`analyze.tabText`·`analyze.tabImage`·`analyze.imageDropzone`·`analyze.imageHint`·`analyze.imagePlaceholder`)는 모두 분석 탭 소속이므로 기존 `analyze.*`에 들어간다. 저장되는 문자열인 `analyze.imagePlaceholder`도 마찬가지다 — 이 값을 만드는 곳이 분석 경로 하나뿐이라, P5가 `persona.create.imagePlaceholder`를 생성 시트 영역에 둔 것과 같은 판단이다(§3.8, [DESIGN §10.1](./DESIGN.md)). 재사용하는 키는 `toast.addImage`·`toast.imageLoadFail`(P5에서 이미 만들었고, **P10 이후 이 둘의 유일한 사용처가 분석 탭이다**).
- **P10도 새 영역을 만들지 않는다.** 신규 키(`persona.create.attachFile`·`attachHint`·`attachedInfo`·`attachedInfoTrimmed`)는 생성 시트 소속이라 기존 `persona.create.*`에, 파일 읽기 실패 알림(`toast.chatFileReadFail`)은 `toast.*`에 들어간다. 대신 **키를 지우는 첫 단계**이기도 하다 — 생성 시트의 이미지 모드가 사라지면서 `persona.create.tabText`·`tabImage`·`imageDropzone`·`imageHint`·`imagePlaceholder` 다섯 키가 쓰이지 않게 되므로 함께 제거한다. `analyze.*`의 같은 이름 키들과 `toast.addImage`·`toast.imageLoadFail`은 **분석 탭이 계속 쓰므로 남긴다.** 표는 [DESIGN §10.1](./DESIGN.md).
...
| `OnboardingModal` | 키 `password` 입력 + 발급 링크(`GEMINI_API_KEY_HELP_URL`) + 동의 체크박스(`onboarding.consent` — Gemini 전송·로컬 데이터 복구 불가 고지; 문구 수준은 §10 #8) → 저장. 빈 키/미동의는 토스트로 거부. 키가 있으면 `null` | `useApp.setApiKey` |
...
| `PersonaPage` | 목록(`listPersonaSummaries`) · 생성 바텀 시트(이름·나의 이름 + **대화 textarea 하나 + .txt 첨부 버튼 + 사용 글자수 안내** → 제출 전 검증(§3.7) → `createPersona`) · 상세 모달(`PERSONA_FIELDS` 11항목 = summary 블록 + 10 카드, 추가 키는 관대 표시, 나/상대 탭 → `getPersona`, **"추가 대화로 업데이트" 입력 + 버튼 → `updatePersona`**) · 삭제(`removePersona`) · "분석하기로" 진입(`setSelectedPersonaId`) | `lib/persona.ts`, `lib/chatFile.ts`, `lib/config.ts` |
...
- **`PersonaPage` 생성 시트의 P10 상태**(P5의 입력 모드·`InlineImage[]`를 대체한다): 대화 텍스트와 **첨부 안내 문구**(`string | null`) 둘뿐이다. **시트를 닫아도 버려지지 않는다** — `CreatePersonaDialog`는 `open`이 거짓일 때 `null`을 렌더할 뿐 마운트된 채 남으므로 이름·나의 이름·대화·첨부 안내가 그대로 유지되고 다시 열면 복원된다. 사라지는 시점은 생성 성공 직후의 `reset()`과 `PersonaPage` 언마운트(탭 전환) 둘뿐이다(DESIGN §5.2 취소 행·§9 "입력 유지"). 흐름은 한 방향이다 — 파일 선택 → `FileReader.readAsText` → `parseKakaoChatTail(raw, PERSONA_CHAT_TAIL_CHARS)`(§3.15) → **textarea 값 교체** + 안내 문구 설정. 그 뒤로 첨부라는 사실은 남지 않는다: 제출 시 나가는 것은 textarea의 현재 값 하나이며, `File` 객체도 원본 텍스트도 들고 있지 않는다.
  - **사용자가 textarea를 직접 편집하면 안내 문구를 지운다.** 문구가 말하는 "최근 N자를 사용했어요"는 첨부 직후의 사실이고, 편집한 뒤에는 참이 아니기 때문이다.
  - **파일 입력의 `value`는 읽은 직후 비운다.** 비우지 않으면 같은 파일을 다시 고를 때 `change` 이벤트가 발생하지 않아 첨부가 조용히 무시된다 — 입력란을 편집했다가 원본으로 되돌리는 흐름에서 실제로 걸린다(PRD FR-7).
  - 읽기 실패(`reader.onerror`)는 `toast.chatFileReadFail`로 알리고 **입력란은 건드리지 않는다.**
  - 상세 모달의 "추가 대화로 업데이트" 입력란에는 첨부 버튼을 두지 않는다. 그 자리는 "새로 나눈 대화 몇 줄"을 받는 곳이라 파일 단위 입력과 맞지 않고, 이어 붙이기(§3.7)는 기존 대화에 더하는 동작이라 말미 컷과 의미가 겹친다.
...
- 비동기 실패는 각 화면이 `catch`해 `pushToast(err.message, 'error')`로 표시한다. 유스케이스는 이미 사용자 언어의 메시지를 담은 `Error`를 던진다(§4). **예외 하나** — `updatePersona`·`analyzeReply`가 페르소나 레코드를 못 찾을 때 던지는 "페르소나를 찾을 수 없습니다."는 ko 고정 문자열이다(i18n 키 없음). 칩·상세 모달이 목록에서 고른 id로만 호출하므로 정상 흐름에서는 도달하지 않는 방어 분기라 그대로 두었고, 키로 빼는 정리는 §10 #33.
...
- **`AnalyzePage`의 P9 상태도 화면 로컬이다**: 입력 모드(`'text' | 'image'`)와 선택한 `InlineImage[]`. 흐름은 한 방향이다 — 파일 선택 → `fileToInlineImage`(§3.4.1) 변환 → 썸네일 표시 → 제출 시 `analyzeReply`의 `images`로 전달. **P10 이후 이 형태를 가진 화면은 여기 하나다**(생성 시트는 §3.15의 텍스트 첨부로 바뀌었다). **전역 스토어에도, `drafts.ts`에도 이미지를 올리지 않는다.** 드래프트는 스레드 텍스트 전용이며(§3.12), 캡처는 페르소나 칩을 바꿀 때 스레드·수동 타겟과 함께 비운다 — 다른 상대의 대화 캡처를 들고 갈 이유가 없다. 두 모드의 입력값은 모드 전환만으로는 지우지 않는다(잘못 누른 사용자가 입력을 잃지 않도록 — [DESIGN §6.1](./DESIGN.md)).
...
### 3.15 `src/lib/chatFile.ts` — 카카오톡 대화 파일 말미 파서 (P10에서 생성)

```ts
/**
 * 카카오톡 내보내기(.txt) 또는 평문 대화 텍스트를 받아,
 * 선두 머리말을 떼고 말미 maxChars 자만 잘라 돌려준다.
 * 순수 함수 — DOM·네트워크·LLM을 쓰지 않는다.
 */
...
| 순서 | 규칙 |
|---|---|
| 1 | **줄바꿈 정규화** — `\r\n`·`\r`를 모두 `\n`으로 바꾼다. 카카오톡 내보내기는 환경에 따라 CRLF로 나오는데, 이후 단계가 모두 `\n` 기준이라 여기서 한 번에 맞춘다 |
| 2 | **선두 머리말 제거** — 첫 줄부터 아래 패턴에 걸리는 동안 한 줄씩 떼어낸다. 패턴에 걸리지 않는 첫 줄을 만나면 **멈춘다**(문서 중간의 날짜 구분선까지 지우지 않는다). 머리말 사이의 빈 줄은 함께 흡수한다 |
| 3 | **말미 컷** — 남은 본문이 `maxChars` 이하이면 그대로 쓴다. 넘으면 뒤에서 `maxChars` 자를 취하고, 그 조각이 줄 중간에서 시작했을 수 있으므로 **첫 `\n` 이후부터** 다시 시작해 잘린 반쪽 줄을 버린다 |
| 4 | **앞뒤 공백 `trim`** 후 반환 |
...
| 패턴 | 예 |
|---|---|
| `…님과 카카오톡 대화`(PC 내보내기) / `…님과의 (카카오톡 )대화`(모바일) | `지수 님과 카카오톡 대화`, `지수님과의 카카오톡 대화` |
| `저장한 날짜 :` 로 시작 | `저장한 날짜 : 2026-09-05 21:14:02` |
| `Date Saved :` 로 시작(대소문자 무시) | `Date Saved : 2026-09-05 21:14:02` |
| 양끝을 하이픈 3개 이상으로 감싼 `YYYY년 … ` 날짜 구분선 | `--------------- 2026년 9월 5일 금요일 ---------------` |
| 빈 줄 | (머리말 블록 사이의 공백 줄) |

- **패턴에 걸리지 않으면 원문을 그대로 통과시킨다.** 사용자가 카카오톡 파일이 아닌 평범한 텍스트 파일을 골라도 동작이 깨지지 않아야 하고, 그때 옳은 행동은 "아무것도 지우지 않기"다. 같은 이유로 **모든 줄이 머리말처럼 보이면**(머리말만 있는 파일) 원문을 그대로 둔다 — 전부 지워 빈 문자열을 돌려주는 것보다 낫다.
- **화자 라벨과 타임스탬프는 지우지 않는다.** `[지수] [오후 9:12] 밥 먹었어?` 같은 줄에서 라벨은 모델이 화자를 가르는 근거이고(`buildPersonaPrompt`가 "누가 말했는지"를 요구한다), 타임스탬프는 대화 흐름의 간격을 보여 준다. 이 모듈은 **자르기만 하고 다시 쓰지 않는다.**
- **`maxChars`를 인자로 받고 상수를 직접 읽지 않는다.** 상수는 `config.ts`(§3.2)에 있고 화면이 넘긴다. 그래야 이 모듈이 설정에 의존하지 않는 순수 함수로 남아 테스트에서 작은 값(예: 10)으로 경계 동작을 확인할 수 있다.
- **분석 탭은 이 모듈을 쓰지 않는다.** 분석의 입력은 최근 몇 줄이라 자를 것이 없고, 캡처 이미지 모드는 텍스트를 거치지 않는다(ADR-9·ADR-10).
- 분기가 비자명해 vitest 단위 테스트 대상이다(§9.2) — `thread.ts`가 그랬던 것과 같은 이유이며, 이 모듈은 그중에서도 **경계 조건**(정확히 `maxChars`일 때, 줄바꿈이 하나도 없을 때)이 눈으로 확인하기 어렵다.
...
- **요청 경로별 파라미터**
  ...
  |---|---|---|---|---|
  | 텍스트(페르소나 생성·메시지 분석) | 프롬프트 문자열 | `TEXT_REQUEST_TIMEOUT_MS` 60s | `TEXT_MODEL` | 소형 1.94s / 페르소나 생성 6.57·5.87·4.88·5.63·5.57s / 분석 3.49·3.86·4.51s / 페르소나 업데이트 6.38s (각 1회, §9.10) |
  | 캡처 이미지(**메시지 분석 전용** — P10 이후) | `[{ role:'user', parts:[{text}, …{inlineData}] }]` | `IMAGE_REQUEST_TIMEOUT_MS` 180s | `TEXT_MODEL`(동일) | 2.75s(P9, 캡처 1장, 표본 1). 페르소나 생성 경로의 4.95s(P5)는 **P10에서 사라진 경로의 값**이라 참고용으로만 남긴다 |

  **P10 이후 페르소나 생성은 첨부 파일을 써도 첫 번째 행을 탄다.** 파일은 화면에서 잘려 프롬프트 문자열에 녹아들 뿐이고, `generate`는 그것이 파일에서 왔는지 알지 못한다(§3.15). 말미 16,000자는 UTF-8 약 46 KB이며 **이 크기의 지연은 아직 재지 않았다**(§10 #29).
...
| ADR-10 | **페르소나 생성을 텍스트 전용으로 되돌리고 입력 경로를 카카오톡 대화 파일(.txt) 첨부로 교체** (`chatFile.ts` + `PERSONA_CHAT_TAIL_CHARS` 신설, `CreatePersonaInput.images`와 `buildPersonaPrompt` 이미지 분기 삭제. ADR-6을 대체하고 ADR-9는 건드리지 않는다) | ADR-6의 결론 그대로 — 캡처 모드는 편리하고 텍스트로 넣을 수 없는 대화를 구제한다. P5 관찰이 아쉬웠다면 "여러 장을 시간 순서대로 올리라"는 힌트를 강화하면 된다 | **분량과 페이로드를 계산해 대조했다.** ① 페르소나 프롬프트는 실제 발화 인용을 강제하므로(FR-8) 정확도는 **분량**에 달렸고, P5 실측이 그 방향의 신호를 냈다 — 캡처 1장의 `vocabulary_examples` 5개 중 2개가 음식 명사, 텍스트 38줄에서는 6개가 모두 어미·감탄 표현(각 표본 1). ② 담기는 양: 캡처 1장은 화면 한 장 분량 **약 10~15개 메시지**(추정, OCR 미측정), .txt 말미 16,000자는 샘플 평균 32.6자/메시지 기준 **약 490개** — 두 자릿수 차이다. ③ 페이로드는 반대 방향이다: 캡처 1장 원본 162,080 B → base64 **약 211 KiB**(= 216 kB), 텍스트 16,000자 **약 46 KB**로 **이미지가 약 4.5배 무겁다.** 무거운 쪽이 적게 담는다. ④ "여러 장 올리기"로 메우려면 사용자가 스크롤·캡처를 수십 번 반복해야 하는데, 카카오톡은 **대화 전체를 .txt로 내보내는 기능을 이미 제공한다.** ⑤ 그렇다고 분석 탭까지 뒤집지는 않는다 — 분석이 필요로 하는 것은 단기 맥락이라 화면 한 장 분량이 맞는 크기다(ADR-9의 두 번째 공격). ⑥ **반증하지 못한 것**: 같은 대화를 두 경로로 넣어 비교한 표본은 없다. 정확도 차이 자체는 **정성 판단**이다(§10 #16) | 페르소나 생성의 이미지 경로를 **제거하고** 텍스트 단일 흐름 + .txt 첨부(머리말 제거 + 말미 `PERSONA_CHAT_TAIL_CHARS`=16,000자 컷, 줄 경계 보존)로 교체한다. 살아남은 근거는 ③의 비대칭 하나다. **삭감은 입력 DTO와 프롬프트 분기에 한정한다** — 저장 스키마·`DB_VERSION`은 그대로이고, 멀티모달 인프라(`InlineImage`·`image.ts`·`generate`의 두 번째 인자·이미지 타임아웃)는 분석 탭이 쓰므로 남는다. 되돌리려면 `CreatePersonaInput`에 선택 필드를 다시 얹으면 된다. 정본은 [PRD §8 부속 결정 3](./PRD.md#8-아키텍처-방향-결정-3단-사고)의 재검토 |

**ADR 최종 상태(P11).**

| ID | 상태 |
|---|---|
| ADR-1 Client-First | **유효** — 전 단계에서 전제가 유지됐다 |
| ADR-2 단일 flash 모델 + thinking off | **유효** — 실키 14회 모두 수락. lite 선택 근거와 thinking off 효과는 미실측(§10 #1) |
| ADR-3 IndexedDB + 쿠키(키) | **부분 대체** — 레코드를 IndexedDB에 두는 결정은 유효, 키 부분은 ADR-8이 대체 |
| ADR-4 HashRouter | **유효** — 하위 경로 배포에서 404 리라이트가 필요 없었다 |
| ADR-5 Zustand 최소 전역 상태 | **유효** — 스토어는 끝까지 하나(`apiKey`·`selectedPersonaId`·`toasts`) |
| ADR-6 페르소나 생성의 캡처 이미지 모드 | **대체됨** — ADR-10이 뒤집었다. 이 결정이 만든 멀티모달 인프라는 ADR-9가 계속 쓴다 |
| ADR-7 분석 입력 계약 재설계 | **유효** — 다만 종합에 적었던 `analyzeMessage` 래퍼 존치는 P11에서 제거로 종결 |
| ADR-8 API 키 localStorage 전환 | **유효** — 전환 후 프로브 재실행에서 키를 실은 요청 0건 |
| ADR-9 분석 입력의 캡처 이미지 모드 | **유효** — 감수한 기능 후퇴(앱이 답장 대상을 모름)는 미확정 관찰 항목으로 남는다(§10 #27) |
| ADR-10 페르소나 생성을 텍스트 전용 + .txt 첨부로 교체 | **유효** — 삭감이 분석 탭으로 새지 않았음을 P10 검증에서 확인 |

**뒤집힌 결정은 둘(ADR-3의 키 부분, ADR-6)이고, 두 번 다 근거는 새 취향이 아니라 재서 얻은 수치였다.**
...
const BASE_PATH = '/persora';                  // vite.config.ts의 base와 같아야 한다(P8)
...
app.get('/', (_req, res) => res.redirect(302, `${BASE_PATH}/`));         // 루트 → base
app.use(BASE_PATH, express.static(DIST_DIR, { index: 'index.html', maxAge: '1h' }));
app.get(`${BASE_PATH}/*`, (_req, res) => res.sendFile(join(DIST_DIR, 'index.html'))); // SPA 폴백(안전망)
```

- **dist/를 루트가 아니라 `base` 하위(`/persora/`)에 마운트한다**(P8에서 정정). 루트에 그대로 마운트하면 빌드본 `index.html`이 참조하는 `/persora/assets/…` 경로가 404가 되어 앱이 깨진다. 도메인 루트로 들어온 요청은 302로 `/persora/`에 보낸다.
...
- 시작 로그에 `http://localhost:8000/persora/`와 "같은 네트워크의 휴대폰: http://[이 PC의 IP]:8000/persora/"를 출력한다.

### 6.3 포트

| 용도 | 명령 | 호스트:포트 |
|---|---|---|
| 개발(HMR) | `npm run dev` | `0.0.0.0:4121` → 앱은 `http://localhost:4121/persora/` (`strictPort`) |
| 빌드 미리보기(Vite) | `npm run preview` | `0.0.0.0:8000` → `/persora/` |
| 빌드 미리보기(Express) | `npm start` | `0.0.0.0:8000` → `http://localhost:8000/persora/`, `/`는 302 리다이렉트 (`PORT`/`HOST` env로 변경) |

세 경로 모두 `base`가 `/persora/`라 하위 경로에서 앱이 뜬다. 도메인 루트에는 Express만 리다이렉트를 둔다.
...
| `npm run dev` | 개발 서버 `http://localhost:4121/persora/` |
| `npm run build` | 타입 검사 통과 후 `dist/` 생성. **둘 중 하나라도 실패하면 빌드 실패** |
| `npm start` | `dist/`를 8000 포트의 `/persora/`에 서빙(A5) |
| `npm test` | `vitest run` — 순수 모듈 단위 테스트 45개(§9.2). P6에서 도입 |
...
| **데이터 전송 고지** | 사용자가 대화가 어디로 가는지 모름 | 고지 내용: 페르소나 생성 시 **대화 텍스트**(붙여넣었든 .txt 첨부로 채웠든 입력란의 그 텍스트 — 파일이 통째로 올라가지 않는다), 분석 시 페르소나 JSON + 최근 대화 스레드 **또는 첨부한 캡처 이미지**가 **Google Gemini API로 직접 전송**되며, 우리 서버는 어떤 개인 데이터도 받지 않는다. 캡처는 대화 본문 외의 부수 정보(프로필 사진·표시 이름 등)까지 함께 실려 나간다는 점을 **분석 탭 이미지 모드 힌트**에 적는다(PRD DR-4 — P10 이후 캡처를 보내는 화면은 여기 하나다). 브라우저 데이터 삭제 시 복구 불가와 백업 수단도 함께 고지. 노출 위치는 **온보딩 모달(최초 1회) + 설정 탭(상시 6항목)** 으로 확정(PRD FR-38) |
...
| **의존성** | 공급망 | 런타임 의존성 최소(react, react-dom, react-router-dom, zustand, @google/genai, express, compression). 버전은 `package.json`에 고정. `npm audit`은 **P8에서 실행했다** — `npm audit fix`(비강제) 후 12건 → **7건(모두 moderate)** 이 남았고, 남은 항목은 major 업그레이드가 필요하며 런타임 도달 경로가 없다고 판단해 수용했다(§10 #26). 재점검 시점은 의존성 major 업그레이드 때다 |
...
- **개발 서버(Vite HMR)와 충돌하지 않는다(P8 확인).** `script-src 'self'`가 HMR 클라이언트와 부딪힐 수 있다고 보고 `npm run dev`와 빌드본 양쪽을 열어 콘솔을 확인했다 — 앱 렌더 정상, CSP 위반 메시지 0(Chromium 실측, §10 #22).
...
**P10에서 `src/lib/chatFile.test.ts`가 더해진다.** 같은 트리거("분기가 비자명한 순수 모듈")를 `parseKakaoChatTail`(§3.15)이 다시 충족한다 — 머리말 패턴 다섯 가지, 패턴 불일치 시 원문 보존, 말미 컷의 줄 경계 처리, 컷이 일어나지 않는 짧은 입력, CRLF 정규화가 모두 눈으로 훑어 맞다고 말할 수 없는 분기다.

| 파일 | 대상 |
|---|---|
| `src/lib/chatFile.test.ts` | `parseKakaoChatTail` — ① 카카오톡 머리말(대화 제목·저장한 날짜·`Date Saved`·날짜 구분선)이 제거되고 첫 대화 줄부터 남는지 ② 머리말이 없는 평문은 **한 글자도 잘리지 않고** 통과하는지 ③ `maxChars`를 넘으면 말미만 남고 **잘린 반쪽 줄이 버려지는지**(결과가 온전한 줄로 시작) ④ `maxChars` 이하인 짧은 입력은 그대로(trim만) 나오는지 ⑤ CRLF 입력이 LF로 정규화되는지 |
...
| A6 모바일 동일 동작 | 같은 Wi-Fi 휴대폰에서 `http://[PC IP]:8000/persora/` 접속, A1~A4 반복 |
...
### 9.7 P8 검증(계획 — 7번을 뺀 전부를 실행했다. 결과 수치는 [`LOG.md`](./LOG.md))
...
### 9.8 P9 검증(계획 — 모두 실행했다. 결과 수치는 [`LOG.md`](./LOG.md))
...
### 9.9 P10 검증(계획 — 모두 실행했다. 결과 수치는 [`LOG.md`](./LOG.md))

| # | 항목 | 방법 | 통과 기준 |
|---|---|---|---|
| 1 | 게이트 | `npm test` · `npx tsc --noEmit` · `npx vite build` | 0 실패 / 0 에러 / 성공. **신규 `chatFile.test.ts`가 테스트 수에 더해진다** |
| 2 | **파서 단위 검증** | §9.2의 다섯 케이스 | 머리말 제거·평문 통과·줄 경계 보존·짧은 입력 무변경·CRLF 정규화가 모두 통과 |
| 3 | **첨부 실측(머리말·컷)** | 실제 카카오톡 대화 내보내기 `.txt`를 첨부 | 입력란이 **대화 첫 줄부터** 채워지고(머리말 없음), 파일이 상한보다 길면 "원본 N자 중 최근 M자" 안내가 뜨며 M ≤ 16,000. 첫 줄이 **온전한 한 줄**이다. 원본 글자 수와 채워진 글자 수를 LOG에 기록한다 |
| 4 | **같은 파일 재첨부** | 첨부 → 입력란을 편집 → 같은 파일을 다시 첨부 | 두 번째 첨부에서도 입력란이 다시 채워진다(파일 입력 `value` 초기화 확인) |
| 5 | 텍스트 붙여넣기 무회귀 | 파일을 쓰지 않고 P5 이전처럼 대화를 붙여넣어 생성 | 20자 미만 거부 토스트, 정상 생성, 상세의 원본 대화가 붙여넣은 그대로 |
| 6 | **분석 탭 무회귀** | 분석 탭의 텍스트/캡처 이미지 모드를 그대로 사용 | 세그먼트·드롭존·썸네일·플레이스홀더 저장이 P9와 동일하게 동작한다. **페르소나 쪽 삭감이 분석 탭에 새지 않았는지**가 이 항목의 목적이다 |
| 7 | **실키 생성 1회** | 3번에서 채운 텍스트로 페르소나 생성 | 11필드 JSON 파싱 성공. **지연을 Resource Timing으로 실측**해 기록하고, `vocabulary_examples`가 어미·표현 인용으로 채워지는지 **눈으로 관찰**한다. 표본 1이므로 정확도를 주장하지 않고 §10 #16·#29에 사실만 적는다 |
| 8 | 기존 레코드 표시 | P5~P9에 캡처로 만든 페르소나가 있으면 상세를 연다 | 플레이스홀더 문자열이 그대로 보이고 화면이 깨지지 않는다 |

### 9.10 최종 검증 요약 (P11)

§9.5가 M1까지의 기록이라면 이 절은 **P11 시점의 최종 상태**를 모은 것이다. 근거는 모두 [`LOG.md`](./LOG.md)이며, 여기 없는 수치는 쓰지 않는다.

**게이트** — P11 ① 시점 기준.

| 항목 | 결과 |
|---|---|
| `npm test` | **45 통과 / 45**(5 파일: `thread` 12 · `drafts` 18 · `chatFile` 7 · `gemini` 4 · `id` 4) |
| `npx tsc --noEmit` | **0 에러** |
| `npx vite build` | **성공** — JS `index-DqBYvWdh.js` 557.70 kB(gzip 140.26 kB) |
| 정적 서빙 | `node server/index.js` → `/persora/` 200, `/` 302 → `/persora/`, `/persora/app-logo.png` 200 |
| 주석 위생 grep | 해당 없음(0건) |

**실키 호출** — 전 단계를 통틀어 **14회**이며, 지연은 모두 브라우저 Resource Timing 기준 **각 1회** 측정값이다.

| 경로 | 지연 | 단계 |
|---|---|---|
| 소형 프롬프트(한 문장 + JSON-only) | 1.94s | P2 |
| 페르소나 생성(텍스트) | 6.57s · 5.87s · 4.88s · 5.63s · 5.57s | P3 · P4 · P9 · P10(15,989자) · P10(1,238자) |
| 페르소나 생성(캡처 이미지 1장) | 4.95s | P5 — **P10에서 사라진 경로**의 값이라 참고용 |
| 메시지 분석 | 3.49s · 3.86s · 4.51s · 2.75s | P4(v1) · P6-1(`decline` 의도) · P6-2(타겟 피커) · P9(캡처 1장) |
| 페르소나 업데이트(추가 대화) | 6.38s | P6-2 |

위 12회 외에, P6 재설계의 반증 실측으로 v1 분석 함수를 직접 호출한 2회(**3.52s** 상대 발화로 끝나는 스레드 / **3.84s** 마지막 줄이 내 발화)가 있다([PRD §8 부속 결정 4](./PRD.md)). 모든 호출이 정상 응답을 돌려줬고 JSON 파싱에 성공했다. **표본이 각 1건이라 분산·중앙값·준수율은 말할 수 없다**(§10 #1).

**전용 프로브**

| 프로브 | 결과 | 단계 |
|---|---|---|
| 키 전송(요청별 `Cookie` 헤더 로깅) | 쿠키 시절 — 저장 전 4건 중 0건, 저장 후 **5건 중 5건**. localStorage 전환 후 재실행 — **5건 중 0건** | P8 |
| CSP 위반 | 빌드본·dev 서버 양쪽 콘솔에서 0건 | P8 |
| `npm audit` | `audit fix`(비강제) 후 12건 → **7건(모두 moderate)**, 수용 | P8 |
| 오버레이 배치 | 포털 적용 후 `getBoundingClientRect().top = 0`, `elementFromPoint(200, 2)`가 오버레이 | P7-3 |
| 비보안 컨텍스트 uuid | LAN IP(`http://192.168.47.1:4121`)에서 `crypto.randomUUID` undefined인 채 `uuid()` 5회 모두 v4 형식 | P7-2 |

**돌리지 않은 것** — 실기기 접속(A6)과 배포 URL에서의 A1~A4 재확인은 끝까지 **미실행**이다. `dataManagement.ts`·`image.ts`·React 화면·IndexedDB CRUD는 단위 테스트 대상이 아니며 브라우저 스모크로만 확인했다(§9.2).

---

## 10. 미확정 항목

P11에서 전 항목을 다시 훑었다. **취소선이 그어진 행은 종결**(결과만 남긴다)이고, 나머지가 **남은 미확정**이다. 남은 항목의 확정 방법·시점은 같은 내용이 [PRD §11.2](./PRD.md)와 [PLAN §8](./PLAN.md)에도 단일 목록으로 유지된다.

| # | 항목 | 확정 시점/방법 |
|---|---|---|
| 1 | `gemini-3.1-flash-lite` + `thinkingBudget=0`의 지연·JSON 준수율·페르소나 품질 | **부분 확인(M1)**: 실키 4회 모두 정상 응답·JSON 파싱 성공, 지연 1.94s/6.57s/5.87s/3.49s(§9.5). 남은 미확정 — 실키 호출은 최종 14회로 늘었지만(§9.10) 시나리오별 표본은 여전히 각 1회라 분산·준수율을 말할 수 없고, **thinking off의 효과는 off 상태만 재서 미실측**이다. 실사용에서 표본을 늘린다 |
...
| 12 | 첫 로드 JS에 코드 스플리팅을 도입할지 | **도입하지 않고 배포한다.** 최종 빌드 기준 JS 557.70 kB(gzip 140.26 kB)이고 대부분이 `@google/genai` 번들이다. 정적 호스트에서 1회 로드 후 캐시되는 자산이라 체감 비용을 아직 재지 않았고, 근거 없이 분할하면 LLM 호출 경로만 늦어질 수 있다. 배포 후 실제 로드 시간을 보고 판단한다 |
...
| 14 | 실기기 확인(A6, 같은 Wi-Fi 휴대폰) | **미실행.** 자동화 뷰포트 390/360px와 PC 브라우저의 LAN IP 재현(P7-2)만 했고 실기기 접속은 끝까지 하지 않았다. 방법: `npm start` 후 휴대폰에서 `http://[PC IP]:8000/persora/` 접속해 A1~A4 반복(§9.4). 시점: 실기기 확보 시 |
| 15 | 캡처 이미지 요청의 지연·페이로드 크기와 `IMAGE_REQUEST_TIMEOUT_MS = 180_000`의 적정성 | **부분 실측(표본 각 1).** 페르소나 생성 4.95s(P5 — 캡처 1장, 원본 162,080 B → base64 약 211 KiB = 216 kB), 분석 2.75s(P9 — 캡처 1장, 크기 미기록). 둘 다 180초에 한참 못 미치지만 표본이 1건씩이라 값은 그대로 둔다. **P10 이후 이 상수가 걸리는 경로는 분석 탭 하나**이고, 여러 장 첨부의 지연은 여전히 미실측이다 |
...
| 23 | ~~의존성 취약점(`npm audit`) 실행 여부~~ **실행함(P8)**: 결과와 판단은 #26에 적었다. 이 행은 중복이라 종결한다 | P8 완료 |
| 24 | 배포 후 Acceptance A1~A4 재확인 | **미실행.** Pages URL은 `base '/persora/'` 하위 경로라 자산 경로·HashRouter·저장소 origin이 로컬과 달라지는 첫 환경이다. `main` push 이후에만 확인할 수 있다(§9.7 7번). 시점: `main` push 후 |
| 25 | 백업 스키마 `version`을 올릴 기준 | 미확정 — 지금은 1. 레코드 필드는 계속 선택 필드로 가산되므로 구 백업이 그대로 읽힌다. 읽을 수 없게 되는 변경이 생길 때만 올리고, 그때 마이그레이션을 어떻게 할지 정한다 |
| 26 | 의존성 취약점(npm audit) | **P8 기록**: `npm audit fix`(비강제) 후 12건 → 7건(모두 moderate, major 업그레이드 필요: vite/esbuild, express/qs, react-router). express/qs는 로컬 미리보기 서버 전용(번들 미포함), react-router 건은 HashRouter·고정 경로·SSR 없음으로 미사용 경로 → 수용. 다음 major 업그레이드 시 재점검 | 수용(재점검 시점: 의존성 major 업그레이드) |
| 27 | **이미지 모드 분석의 답장 대상 판별 정확도**(§3.5·ADR-9) | **미확정.** 캡처만 넣으면 앱이 타겟을 모르고 프롬프트가 모델에게 "맨 아래(최신) 상대 메시지를 찾아라"고 위임한다. 말풍선 좌/우 위치와 순서만으로 얼마나 맞히는지 표본이 없고, 틀려도 사용자가 고칠 수단이 없다(`targetOverride`는 텍스트 모드 전용). 완화책은 텍스트 모드를 기본으로 남겨 두는 것과 화면 힌트뿐이다. **P9 검증의 표본 1건에서는 캡처 맨 아래 상대 메시지에 정확히 답했으나 적중률을 주장하지 않는다.** 방법: 실사용에서 오판 사례를 모으고, 잦으면 DESIGN U29·U30의 후보를 검토한다. 시점: 실사용 관찰 |
| 28 | ~~**분석 경로 이미지 요청의 지연**과 `IMAGE_REQUEST_TIMEOUT_MS`(180초)의 적정성~~ **실측(P9, 표본 1)**: 캡처 1장 분석 2.75s. 180초 안이므로 상수는 그대로 둔다. 표본이 1건이라 분산은 모른다 | P9 완료(부분) |
| 29 | **첨부 대화 파일의 말미 컷 상한 `PERSONA_CHAT_TAIL_CHARS = 16_000`의 적정성**(§3.2·§3.15) | **부분 실측(P10, 각 1회).** 지연은 쟀다 — 상한까지 채운 15,989자 생성 5.63s, 머리말만 제거된 1,238자 생성 5.57s로 분량이 10배 차이인데도 거의 같았다. **남은 미확정은 토큰 사용량과 페르소나 품질이다.** 값의 근거는 여전히 계산 하나(샘플 평균 32.6자/메시지 기준 약 490개 메시지, UTF-8 약 46 KB)이며, 늘리면 인용 재료가 늘지만 컨텍스트 압박이 커지고 줄이면 P5에서 본 "근거가 얇은 페르소나"로 되돌아간다. 방법: 같은 대화로 상한을 바꿔 가며(예: 8,000 / 16,000 / 32,000) 인용 재료의 양과 응답 품질을 비교하고, 조정은 `config.ts` 한 곳. 시점: 실사용 관찰 |
| 30 | 카카오톡 내보내기 머리말 패턴의 실제 적중률(§3.15) | **미확정.** P10 검증에서 PC 내보내기 제목("OOO 님과 카카오톡 대화", 조사 '의' 없음)이 첫 패턴에 걸리지 않아 머리말이 남는 결함을 발견해 정규식을 넓혔지만, **표본은 여전히 모바일·PC 두 표기뿐이다.** 카카오톡 버전·플랫폼·언어 설정에 따라 형태가 다를 수 있다. 걸리지 않으면 **원문을 그대로 통과시키므로 실패해도 파괴적이지 않고**(머리말 몇 줄이 대화에 섞일 뿐) 사용자가 입력란에서 지울 수 있는 것이 완화책이다. 방법: 다른 환경의 내보내기 파일을 첨부해 첫 대화 줄부터 채워지는지 보고 패턴을 더한다. 시점: 실사용 관찰 |
| 31 | `ErrorBoundary` 복구 화면 문구의 i18n | **미해결.** 문구가 ko 고정 문자열이다 — DESIGN §10.1에 오류 화면 영역이 없어 P7-1에서 키를 만들지 않았다. 렌더 예외를 인위적으로 주입한 적도 없어 복구 UI의 실제 동작도 미확인이다. 방법: i18n 영역을 하나 더할지(`error.*`) `common.*`에 넣을지 정해 ko/en 키를 만들고, 예외를 주입해 화면을 확인한다. 시점: 후속 후보([PLAN §8](./PLAN.md)) |
| 32 | 첨부 안내 문구가 머리말 제거와 tail 컷을 구분하지 않음(§3.10) | **관찰(P10).** 1,331자 파일이 머리말 제거만으로 1,238자가 됐을 때도 `attachedInfoTrimmed`("원본 N자 중 최근 M자만 사용했어요")가 떠, 잘리지 않았는데 잘린 것처럼 읽힌다. 비교 기준이 원본 전체 길이여서 생기는 일이다. 방법: 비교 기준을 머리말 제거 후 길이로 바꾸거나 두 경우의 문구를 나눈다(i18n 키 1~2개 + `PersonaPage`의 조건 한 줄). 시점: 후속 후보 |
| 33 | ko 고정 문자열 잔존(i18n 키 없음 — §3.9 영역 규칙 밖) | **발견(P11 코드 대조).** `AnalyzePage` 타겟 피커의 내 발화 라벨 폴백 `'나'`(`my_name`이 비었을 때, en 모드에서도 한글 — P6-2 LOG가 미확정으로 남겼던 항목)와 `updatePersona`·`analyzeReply`의 not-found `Error` 메시지("페르소나를 찾을 수 없습니다.", §3.10의 예외). 방법: `analyze.me`(ko "나"/en "Me")·`err.personaNotFound` 키를 ko/en 사전에 더하고 세 곳을 교체. 시점: #31과 묶어 후속([PLAN §8.3](./PLAN.md) #2) |
| 34 | ~~설정 탭 고지 문구 2건과 계약 불일치(§8 "데이터 전송 고지"·"키 취급" 행)~~ **종결(P11 ④)**: 두 문구를 코드에서 정정(referrer 절 삭제·API 제한/회전 안내, 캡처 전송 화면을 분석 탭으로) | P11 완료 |
