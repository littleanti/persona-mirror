...
export default function OnboardingModal() {
  ...
  // 검증 순서(DESIGN §4): ① 빈 값 → toast.invalidKeyFormat ② 미동의 → toast.confirmLocalOnly
  // ③ 통과 → 저장. 사전 검증 호출(키 유효성 확인)은 하지 않는다 — TRD §3.10/§4.
  const onSubmit = () => {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm">
      ...
      <div className="relative max-w-md w-full bg-white border border-slate-200 rounded-2xl p-8 shadow-soft-lg animate-fade-in">
        ...
        <p className="text-sm text-slate-600 mb-6 leading-relaxed">{t('onboarding.intro')}</p>
        ...
        <button
          onClick={() => onSubmit()}
          className="w-full py-3.5 rounded-2xl bg-brand-gradient text-white font-semibold text-sm shadow-glow hover:opacity-90 active:scale-[.98] transition-all"
        >
          {t('btn.saveKey')}
        </button>
      </div>
