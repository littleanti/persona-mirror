...
| 29 | **첨부 대화 파일의 말미 컷 상한 `PERSONA_CHAT_TAIL_CHARS = 16_000`의 적정성**(§3.2·§3.15)  — P10 실측: 16,000자 tail(15,989자) 생성 5.63s, 1,238자 생성 5.57s(각 표본 1) — 상한이 지연을 크게 늘리지 않음 | **미실측.** 값의 근거는 계산 하나다 — 샘플 평균 32.6자/메시지 기준 약 490개 메시지, UTF-8 약 46 KB. 이 분량에서 **토큰 사용량·응답 지연·페르소나 품질을 재 본 적이 없다.** 늘리면 인용 재료가 늘지만 지연과 컨텍스트 압박이 커지고, 줄이면 P5에서 본 "근거가 얇은 페르소나"로 되돌아간다. P10 검증(§9.9 3·7번)에서 실제 `.txt` 1개로 생성해 원본 글자 수·사용 글자 수·지연을 기록하고, 값이 과하거나 모자라면 `config.ts` 한 곳을 고친다 |
| 30 | 카카오톡 내보내기 머리말 패턴의 실제 적중률(§3.15)  — P10 실측: PC 내보내기 제목("OOO 님과 카카오톡 대화", 조사 없음)이 첫 패턴에 안 맞아 머리말이 남는 결함을 발견·수정(패턴 표 정정). 표본은 여전히 두 표기뿐 | **미확정.** 상정한 것은 다섯 가지(대화 제목·저장한 날짜·`Date Saved`·날짜 구분선·빈 줄)뿐이고, 카카오톡 버전·플랫폼(iOS/안드로이드/PC)·언어 설정에 따라 머리말 형태가 다를 수 있다. 표본은 한 형태밖에 없다. 패턴에 걸리지 않으면 **원문을 그대로 통과시키므로 실패해도 파괴적이지 않고**(머리말 몇 줄이 대화에 섞일 뿐), 사용자가 입력란에서 직접 지울 수 있는 것이 완화책이다. 다른 형태를 만나면 패턴을 더한다 |
