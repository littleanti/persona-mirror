...
// t() 키로 넘겨 폴백(키 문자열 자체를 반환)이 라벨로 보이게 한다(DESIGN §5.3/§10.1).
const FIELD_LABELS: Record<string, string> = {
  communication_style: 'persona.field.communication_style',
  speech_level: 'persona.field.speech_level',
  vocabulary_examples: 'persona.field.vocabulary_examples',
  sentence_style: 'persona.field.sentence_style',
  emoji_symbol_usage: 'persona.field.emoji_symbol_usage',
  texting_habits: 'persona.field.texting_habits',
  emotional_tendencies: 'persona.field.emotional_tendencies',
  what_they_value: 'persona.field.what_they_value',
  how_they_seek_response: 'persona.field.how_they_seek_response',
  relationship_dynamics: 'persona.field.relationship_dynamics',
};
    ...
    >
      <div className="w-full max-w-lg h-[92dvh] flex flex-col bg-white border border-slate-200 rounded-t-3xl shadow-soft-lg animate-slide-up">
        ...
        <div className="flex items-center justify-between px-5 py-3 shrink-0">
          <h3 className="font-semibold text-slate-900 text-base">{translate('persona.create.title')}</h3>
          <button
            onClick={onClose}
        ...
        <div className="px-5 pb-6 flex-1 min-h-0 overflow-y-auto flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-1">
              <span className="text-xs font-medium text-slate-500">{translate('persona.create.otherName')}</span>
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder={translate('persona.create.otherNamePlaceholder')}
            ...
            <label className="space-y-1">
              <span className="text-xs font-medium text-slate-500">{translate('persona.create.myName')}</span>
              <input
                value={myName}
                onChange={(e) => setMyName(e.target.value)}
                placeholder={translate('persona.create.myNamePlaceholder')}
                className="w-full bg-slate-50 border-[1.5px] border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:bg-white transition-colors"
              />
            </label>
          </div>
          ...
          <button
            ...
            disabled={saving}
            className="w-full py-3 rounded-full bg-brand-gradient text-white font-semibold text-sm shadow-glow-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[.98] shrink-0"
          >
            {saving ? translate('persona.create.loading') : translate('persona.createCta')}
          </button>
        </div>
      </div>
    </div>,
...
function PersonaDetailDialog({
  ...
  const myPersonaData = persona.my_persona;
  ...
  const onDelete = async () => {
    if (!window.confirm(translate('persona.detail.confirmDelete', { name: persona.name }))) return;
    try {
    ...
    >
      <div className="w-full max-w-lg max-h-[92dvh] overflow-y-auto bg-white border border-slate-200 rounded-t-3xl shadow-soft-lg animate-slide-up">
        ...
        <div className="flex items-center justify-between px-5 py-3">
          <h3 className="font-semibold text-slate-900 text-base">{translate('persona.detail.title')}</h3>
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors text-lg leading-none"
          >
        ...
        <div className="px-5 pb-6 space-y-4">
          <div className="text-center pt-2">
            ...
            <p className="text-xs text-slate-400 mt-1">{translate('persona.detail.createdAt', { date: formatDate(persona.created_at) })}</p>
            {persona.my_name && (
              <span className="inline-flex mt-2 rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-xs font-semibold text-emerald-600">
                {translate('persona.detail.myLabel', { my: persona.my_name })}
              </span>
            )}
          </div>
          ...
          {hasDual && (
            <div className="flex rounded-2xl bg-slate-100 p-1 text-xs font-semibold">
              ...
              >
                {translate('persona.detail.tabMe', { my: persona.my_name })}
              </button>
            </div>
          )}
          ...
          <div className="rounded-2xl bg-slate-50 border border-slate-200 overflow-hidden">
            ...
            >
              <span>{translate('persona.detail.convToggle')}</span>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-3">
            ...
            >
              {translate('persona.detail.useForAnalysis')}
            </button>
            <button
  ...
  return (
    <section className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-bold text-slate-900">{translate('persona.title')}</h1>
          <p className="text-sm text-slate-500 mt-1">{translate('persona.subtitle')}</p>
        </div>
        <button
          onClick={() => setCreateOpen(true)}
          className="flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold text-white bg-brand-gradient shadow-glow-sm transition-all active:scale-[.98] flex-shrink-0"
        >
          <span>+</span>
          <span>{translate('persona.createCta')}</span>
        </button>
      </div>
      ...
      {!loading && personaCount === 0 && (
        <div className="border-2 border-dashed border-slate-300 rounded-2xl p-10 text-center space-y-4">
          ...
          <div>
            <p className="text-slate-700 text-sm font-semibold">{translate('persona.empty.title')}</p>
            <p className="text-slate-400 text-xs mt-1">{translate('persona.empty.desc')}</p>
          </div>
          <button
            onClick={() => setCreateOpen(true)}
            className="px-5 py-2.5 rounded-full bg-brand-gradient text-white font-semibold text-sm shadow-glow-sm transition-all active:scale-[.98]"
          >
            {translate('persona.createCta')}
          </button>
        </div>
      )}
